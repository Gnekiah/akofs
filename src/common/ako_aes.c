...
inline uint64_t ako_aes_bound(uint64_t size) {
    return ((size >> 4) << 4) + (size & 0x0f ? 0x10 : 0);
}

uint64_t ako_aes_cbc256_encrypt(const uint8_t* src, const uint64_t src_size, uint8_t* dst,
    ...
    if (dst_size < ako_aes_bound(src_size)) {
        return -3;
    }
...
uint64_t ako_aes_cbc256_decrypt(const uint8_t* src, const uint64_t src_size, uint8_t* dst,
    uint64_t dst_size, const char* key) {
    const uint8_t* src_end = src + src_size;
    uint8_t* dst_end = dst;
    uint8_t in[AES_BLOCK_SIZE];

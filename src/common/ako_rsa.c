...
int ako_rsa_generate_keypair_2048(int8_t* pri_key, uint64_t* pri_key_size,
...
uint64_t ako_rsa_pub_encrypt(const uint8_t* src, const uint64_t src_size, uint8_t* dst,
...
uint64_t ako_rsa_pri_decrypt(const uint8_t* src, const uint64_t src_size, uint8_t* dst,
...
/* return sign_size */
uint64_t ako_rsa_pri_sign(const uint8_t* src, const uint64_t src_size, int8_t* sign,
    ...
    uint64_t __base64_size = ako_base64_enbound(__rsa_size);
    ...
    ret = ako_base64_encode(sign_bin, sign_size, sign);
...
int ako_rsa_pub_verify(const uint8_t* src, const uint64_t src_size, const int8_t* sign,
    ...
    uint8_t* debase = malloc(ako_base64_debound(sign_size));
    if (!debase) {
        ret = -1;
        goto free;
    }
    int debase_size = ako_base64_decode(sign, sign_size, debase);

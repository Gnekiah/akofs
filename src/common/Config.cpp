...
ako::Config* __ConfigInit(const std::string& path,
    std::map<std::string, std::string>* config_map) {
    ako::Config* config = nullptr;
    try {
        config = new ako::Config(path, config_map);
    }
...
namespace ako {
    ...
    long Config::Get(const std::string& section, const std::string& name, ako_config_int_t) {
        const std::string val = Get(section, name, AKO_CONFIG_TYPE_STRING);
        return str2type(val, 1);
    }
    double Config::Get(const std::string& section, const std::string& name, ako_config_double_t) {
        const std::string val = Get(section, name, AKO_CONFIG_TYPE_STRING);
        return str2type(val, 0.1);
    }
    bool Config::Get(const std::string& section, const std::string& name, ako_config_bool_t) {
        const std::string val = Get(section, name, AKO_CONFIG_TYPE_STRING);
        return str2type(val, true);
    }
    std::string Config::Get(const std::string& section, const std::string& name, ako_config_string_t) {
        int err = -1;
        std::string key = make_key(section, name);
        std::string val;
